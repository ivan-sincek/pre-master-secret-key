...
Open Wireshark and go to `Edit -> Preferences -> Protocols -> TLS` and load the path to the `ssl-keys.log` into the `(Pre-)Master-Secret log filename` input filed.

Start recording network traffic.

If you suddenly cannot delete the log file, even after closing the web browser, it is possible that some other program is still writing in the log file, e.g OneDrive, Dropbox, etc.
